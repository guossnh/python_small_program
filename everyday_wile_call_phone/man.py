...
def write_result():
    ...
    allday_data = allday_data[["订单号","商品id","手机","店铺","姓名","产品简称"]]
    oneday_data = oneday_data.append(allday_data)
    oneday_data = oneday_data.append(allday_data)
    oneday_data = oneday_data.drop_duplicates(subset=['订单号'],keep=False)
    oneday_data.to_csv(""+man_url+"这里看结果文件\\cha.csv",sep=',',index=False)
    
    new_all_file = pd.merge(allday_data, oneday_data ,how="outer", on="订单号")
    new_all_file = new_all_file.drop_duplicates(subset=['订单号'],keep="first")
    print(new_all_file)
    new_all_file.to_csv(""+man_url+"all_data.csv",sep=',',index=False)


    #oneday_data.to_csv(""+man_url+"all_data.csv",sep=',')

...
product_list =[]
product_file_list =[]
man_URL = "d:\\jisuan\\make_everyday_PDD_date_for_KJ\\"
    ...
    for x in xl.keys():
        if(x.find("组")!= -1):
            ...
            for row in xl.itertuples():
                ...
                product_link_in_list.append(product_link_in(getattr(row, '产品ID'),getattr(row, '店铺'),getattr(row, '产品简称'),getattr(row, '姓名'),0,0,0,0,x,0))
        elif(x.find("数据")!= -1):
            xl = pd.read_excel(""+man_URL+"产品数据表格.xlsx",x)
            for row in xl.itertuples():
...
class product_link_in:
    def __init__(self,pid,shop,ename,pname,car_money,shell_money,sd_money,wb_money,group,count):
        self.pid = pid
        self.shop = shop
        self.ename = ename
        self.pname = pname
...
def writer_file():
    ...
    for one_date in read_config_xlsx():
        try:
            ...
            one_date.count = len(all_date[(all_date["商品id"]==one_date.pid)])
        except:
            pass
    ...
    with open(""+man_URL+"result.csv","w",newline = '') as csvfile: 
        ...
        writer.writerow(["姓名","组","店","产品ID","产品全称","销量","刷单","放单","直通车","单量"])
        for i in product_link_in_list:
            ...
            writer.writerow([i.pname,i.group,i.shop,i.pid,full_name,i.shell_money,i.sd_money,i.wb_money,i.car_money,i.count])

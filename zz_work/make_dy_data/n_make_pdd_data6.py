...
def process_sd_data(sales_data):
    ...
    if not os.path.exists(sd_folder) or not any(os.path.isfile(os.path.join(sd_folder, f)) for f in os.listdir(sd_folder)):
        logging.info("没有找到sd文件夹中的文件，跳过处理刷单数据。")
        # 添加空列
        sales_data['店铺名'] = None
        sales_data['实付金额'] = 0
        sales_data['总服务费'] = 0
    ...
    if not sd_data.empty:
        ...
        # 确保新列存在并填充空值
        merged_data['店铺名'].fillna('没有ID', inplace=True)
        merged_data['实付金额'].fillna(0, inplace=True)
        merged_data['总服务费'].fillna(0, inplace=True)

        return merged_data

    # 如果sd_data为空，直接返回添加了空列的sales_data
    sales_data['店铺名'] = None
    sales_data['实付金额'] = 0
    sales_data['总服务费'] = 0
    return sales_data
...
def determine_order_attribute(row):
    ...
    # 判断是否为刷单
        ...
        return '刷单'
    # 判断是否为放单（基于总服务费是否有值且不为0）
    elif not pd.isna(row['总服务费']) and row['总服务费'] != 0:
        return '放单'
    # 默认情况下为正常订单
    else:
        return '正常'
...
def calculate_id_cost(merged_table):
    """
    计算ID成本：
    使用收货放单、收货正常、退款放单、退款正常相加再减去花费(元)，并生成新的列 ID成本。
    """
    # 计算订单成本的总和
    merged_table['ID成本'] = (
        merged_table['收货放单'] +
...
def main():
    ...
    # 计算ID成本
    merged_pivot_table = calculate_id_cost(merged_pivot_table)
    logging.info("ID成本计算完成")
